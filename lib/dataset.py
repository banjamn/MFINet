...
class Data(Dataset):
    def __init__(self, root, mode='train'):
        ...
        for line in lines:
            ...
            boundary_path = os.path.join(root, 'boundary', line)

            self.samples.append([rgbpath, tpath, dpath, maskpath, boundary_path])
    ...
    def __getitem__(self, idx):
        rgbpath, tpath, dpath, maskpath, boundary_path = self.samples[idx]
        ...
        boundary = cv2.imread(boundary_path).astype(np.float32)
        H, W, C = mask.shape
        if C!=3:
            print("err")
        rgb, t, d, mask_, boundary = self.transform(rgb, t, d, mask, boundary)
        if self.mode == 'train':
            rgb, t, d = getRandomSample(rgb, t, d)
        return rgb, t, d, mask_, boundary,(H,W), maskpath.split('/')[-1],mask
